...
function rememberDefaultLayout() {
  draggableEls.forEach((el) => {
    ...
    const rect = {
      left: parseFloat(cs.left) || el.offsetLeft,
      top: parseFloat(cs.top) || el.offsetTop,
      right: cs.right,
      bottom: cs.bottom,
...
function applyCrop(img, data) {
  // 기본값에서는 이미지 전체가 보이고, 확대하면 그때부터 블럭 안에서 잘려.
  // 그래서 처음부터 얼굴/상반신이 잘린 상태로 들어가는 문제를 줄였어.
  img.style.setProperty('--img-x', `${data.x}px`);
  img.style.setProperty('--img-y', `${data.y}px`);
  img.style.setProperty('--img-z', data.z / 100);
}
...
function makeDraggable(el) {
  ...
  el.addEventListener('pointermove', (event) => {
    ...
    // 회전된 사진 블럭도 드래그 후 회전값을 유지해.
    if (!el.style.transform && defaultLayout.get(el.dataset.key)?.transform) {
      el.style.transform = defaultLayout.get(el.dataset.key).transform;
    }
  });
